...
class SimpleProductSearchHandler(MultipleObjectMixin):
    ...
    def __init__(self, request_data, full_path, categories=None):
        ...
        self.types = request_data.getlist('type')

        self.minparam = {'min': request_data.get('min')}
        self.maxparam = {'max': request_data.get('max')}

        self.tags = request_data.getlist('tags')

        self.object_list = self.get_queryset()

    def get_queryset(self):

        if len(self.types) and self.minparam['min'] is not None:
            qs = Product.objects.browsable().base_queryset().filter(Q(
                product_class__name__in=self.types)).filter(
                stockrecords__price__gte=self.minparam['min']).filter(
                stockrecords__price__lte=self.maxparam['max'])

        if not len(self.types) and self.minparam['min'] is not None:
            qs = Product.objects.browsable().base_queryset().filter(
                stockrecords__price__gte=self.minparam['min']).filter(
                stockrecords__price__lte=self.maxparam['max'])

        if not len(self.types) and self.minparam['min'] is None:
